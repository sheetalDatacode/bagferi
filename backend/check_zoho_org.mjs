...
async function checkOrg() {
    try {
        ...
        const res = await axios.get(`${ZOHO_BOOKS_BASE}/organizations`, {
            headers: {
                Authorization: `Zoho-oauthtoken ${token}`,
            }
        });
        const org = res.data.organizations.find(o => o.organization_id === ZOHO_ORG_ID);
        console.log('Org Details:', JSON.stringify(org || res.data.organizations[0], null, 2));
    } catch (err) {
