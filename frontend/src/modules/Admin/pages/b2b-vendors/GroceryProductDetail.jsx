...
import { FiArrowLeft, FiPackage, FiUser, FiTag, FiShoppingBag } from "react-icons/fi";
import { motion } from "framer-motion";
import api from "../../../../shared/utils/api";
import toast from "react-hot-toast";
import RatingSummaryBadge from "../../../../shared/components/RatingSummaryBadge";

const AdminGroceryProductDetail = () => {
    const { id } = useParams();
    ...
    const toggleStatus = async (approved) => {
        try {
            const response = await api.patch(`/grocery/admin/products/${id}/status`, {
                        ...
                        <div className="flex flex-col md:flex-row md:items-start justify-between mb-8 gap-4">
                            ...
                            <div className="text-left md:text-right bg-gray-50 md:bg-transparent p-4 md:p-0 rounded-2xl">
                                ...
                                {product.mrp && product.mrp > product.price && (
                                    <p className="text-xs font-bold text-gray-400 line-through mt-1">₹{product.mrp}</p>
                                )}
                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">per {product.unit || 'Unit'}</p>
                            </div>
                        </div>
...
export default AdminGroceryProductDetail;
